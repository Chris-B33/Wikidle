...
function cleanWikipediaHTML(html) {
    ...
    document.querySelectorAll(".mw-references-wrap, " +
        ".reflist, " +
        ".references, " +
        "ol.references, " +
        ".reference, " +
        ...
        ".notelist-plain, " +
        ".sistersitebox, " +
        ".mbox-small").forEach(element => element.remove());
    ...
    document.querySelectorAll("sup.reference, " +
        "sup[role='doc-noteref'], " +
        "a[href^='#cite_note-'], " +
        "a[href^='#cite_ref-']").forEach(element => element.remove());
    // Remove navigation/shortcut information that isn't useful for the game.
    document.querySelectorAll(".hatnote, " +
        ".dablink, " +
        ".rellink, " +
        ".shortdescription").forEach(element => element.remove());
    // Turn image links into plain images.
    document.querySelectorAll("a.image, " +
        "a[href^='./File:'], " +
        "a[href^='/wiki/File:']").forEach(link => {
    ...
    document.querySelectorAll("#catlinks").forEach(element => element.remove());
    // Remove edit links.
    document.querySelectorAll(".mw-editsection").forEach(element => element.remove());
    return document.body.innerHTML;
}
/** * Convert Wikipedia article links into game buttons. */
function processWikipediaLinks(container) {
...
function isValidWikipediaLink(link) {
    const href = link.getAttribute("href");
